  ...
  <div class="container page">
    ...
    <form class="search-area" method="get" action="Artists.php?go">
      <div class="search-area-wrapper">
        <input type="text" name="search-text" class="search-field"
          value="<?php if(isset($_GET['submit'])) { echo $_GET['search-text']; }?>">
      ...
      <div class="filter">
        <div class="filter-type">
          <select name="search-category" id="">
            <option value="" <?php if($emptyQuest) { if ($_GET['search-category'] == "Любое") echo 'selected'; }?>>Любое
            </option>
            <option value="Гимнаст"
              <?php if($emptyQuest) { if ($_GET['search-category'] == "Гимнаст") echo 'selected'; }?>>
              Гимнаст</option>
            <option value="Акробат"
              <?php if($emptyQuest) { if ($_GET['search-category'] == "Акробат") echo 'selected'; }?>>
              Акробат</option>
            <option value="Чревовещатель"
              <?php if($emptyQuest) { if ($_GET['search-category'] == "Чревовещатель") echo 'selected'; }?>>
              Чревовещатель</option>
          </select>
        </div>

        <div class="filter-price">
          Цена от <input name="start-price" class="filter-price-input" type="number"
            value="<?php if(isset($_GET['submit'])) { echo $_GET['start-price']; } else echo 0; ?>">
          до <input name="end-price" class="filter-price-input" type="number"
            value="<?php if(isset($_GET['submit'])) { echo $_GET['end-price']; } else echo 9999; ?>">
        </div>
